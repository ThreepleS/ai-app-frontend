...
// === Polyfill: гарантируем Array.prototype.forEach в любом WebView ===
...
// === Глобальный хелпер: aiHubQueryAll (уникальное имя, чтобы исключить
// collision с любыми внешними библиотеками / WebView). Всегда возвращает
// настоящий Array. Для 100% защиты падений при вызове .forEach на пустых
// результатах все вызовы обёрнуты в проверку ниже. ===
...
const APP_VERSION = "20260831_2103";
...
window.addEventListener("error", (e) => {
    ...
    try {
        ...
        if (el) el.textContent = "JS ошибка: " + (e.message || e.error || e) + " @ " + (e.filename || "?") + ":" + (e.lineno || "?");
    } catch {}
    try {
        if ($("#alertModal")) {
...
dialogs.forEach((d) => {
            ...
            aiHubQueryAll(document, ".dialog-item-del").forEach((btn) => {
                btn.addEventListener("click", async (e) => {
                    ...
                    const ok = await window.window.showConfirm("Удалить диалог", "Вы уверены? Этот диалог будет удалён навсегда.");
                    if (!ok) return;
...
async function auth(devId) {
    ...
    if (!inTelegram && !d) {
        ...
        await window.showAlert("Нужно открыть через Telegram", "<i data-lucide='alert-triangle' class='lucide'></i> " + msg);
        setStatus("err");
        return false;
    }
    let res;
    try {
        ...
        if (!tourActive) {
            await window.showAlert("Ошибка подключения", "<i data-lucide='alert-triangle' class='lucide'></i> " + tip);
        } else {
            queuedAuthError = {
    ...
    try {
        ...
        if (!data.ok) {
            ...
            if (!tourActive) {
                await window.showAlert("Ошибка", "<i data-lucide='alert-triangle' class='lucide'></i> " + (data.error || "нет доступа"));
            } else {
                queuedAuthError = {
                    title: "Ошибка",
...
$("#imgfile").addEventListener("change", (e) => {
    ...
    if (f.size > 8 * 1024 * 1024) {
        window.showAlert("Файл слишком большой", "Максимальный размер фото — 8 МБ.");
        e.target.value = "";
        return;
    }
...
$("#lb_reply").addEventListener("click", async () => {
    ...
    try {
        ...
        await window.showAlert("Ошибка", "<i data-lucide='alert-triangle' class='lucide'></i> Не удалось вставить фото: " + String(err));
    }
});
...
async function mbPick(id) {
    try {
        ...
        await window.showAlert("Ошибка", "<i data-lucide='alert-triangle' class='lucide'></i> " + String(e));
    }
    if (window.lucide) lucide.createIcons();
}
...
if (newChatBtn) {
    ...
    vibClick();
    try {
        if (!(await window.showConfirm("Новый диалог", "Текущий диалог будет сохранён. Продолжить?"))) return;
        await autoSaveCurrentDialog();
        await ef("chat", {
            clear: true
        });
...
function window.showConfirm(title, message) {
    return new Promise((resolve) => {
...
async function window.saveLocalHistory() {
    await autoSaveCurrentDialog();
}
...
async function runTour(startStep = 0) {
    ...
    window.window.showConfirm = window.showConfirm;
