...
const APP_VERSION = "20260908_1950";
...
function createMessageNode(role, content, image, stats, msgIdx) {
    const imgHtml = image ?
...
function loadOlderMessagesChunk() {
    ...
    for (let i = newStart; i < activeRenderedStartIndex; i++) {
        const m = msgs[i];
        const node = createMessageNode(m.role, m.content || "", m.image || null, m.stats || null, i);
