...
// === Глобальный хелпер // === Глобальная функция qsa (гарантированно возвращает настоящий Array) ===
...
dialogs.forEach((d) => {
            ...
            if (window.lucide) lucide.createIcons();
            qsa(document, ".dialog-item-del").forEach((btn) => {
            ...
            qsa(document, ".dialog-item-export").forEach((btn) => {
...
async function autoSaveCurrentDialog() {
    ...
    saveTimer = setTimeout(async () => {
        ...
        try {
            ...
            if (saved && saved.name) {
                const items = qsa(document, ".dialog-item");
                items.forEach((item) => {
...
function updateEmptyState() {
    const isEmpty = qsa(box, ".msg").length === 0;
    if (isEmpty) {
...
if (fsEditor) {
    qsa(fsEditor, "[data-close]").forEach((el) => {
        ...
        if (tourActive && localStorage.getItem(TOUR_KEY) === "true") {
            ...
            if (congrats) {
                ...
                const close = () => {
                    ...
                    qsa(congrats, "[data-close]").forEach((b) => b.removeEventListener("click", onOk));
                    closeSettings();
                    runTour(8);
                };
                const onOk = () => close();
                ...
                qsa(congrats, "[data-close]").forEach((b) => b.addEventListener("click", onOk));
            }
        }
    } catch (err) {
        status.style.color = "#e06b6b";
...
qsa(document, ".seg-picker").forEach((picker) => {
...
function clearSearchHighlight() {
    qsa(box, ".search-highlight-frame")
        .forEach((el) => el.classList.remove("search-highlight-frame"));
    qsa(box, ".search-highlight-term").forEach((el) => {
...
function syncSegPickers() {
    ...
    qsa(document, ".seg-picker").forEach((picker) => {
        ...
        qsa(picker, ".seg-btn").forEach((btn) => {
...
function openSettings(tab = null) {
    ...
    loadKeyInfo();
    if (tab) {
        qsa(document, ".stab").forEach(t => t.classList.remove("active"));
        qsa(document, ".tab-content").forEach(c => c.classList.remove("active"));
...
// --- Settings tabs ----------------------------------------------------
qsa(document, ".settings-tabs .stab").forEach((tab) => {
    tab.addEventListener("click", () => {
        vibClick();
        qsa(document, ".settings-tabs .stab")
            .forEach((t) => t.classList.remove("active"));
        qsa(document, ".tab-content")
...
function showConfirm(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            qsa(modal, "[data-close]").forEach((btn) => btn.removeEventListener("click", onCancel));
            modal.querySelector(".modal-backdrop").removeEventListener("click", onCancel);
        };
        $("#confirmOk").addEventListener("click", onOk);
        qsa(modal, "[data-close]").forEach((btn) => btn.addEventListener("click", onCancel));
...
function showAlert(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            modal.classList.remove("open");
            qsa(modal, "[data-close]").forEach((btn) => btn.removeEventListener("click", onClose));
        ...
        qsa(modal, "[data-close]").forEach((btn) => btn.addEventListener("click", onClose));
...
qsa(document, "[data-close]").forEach((btn) => {
    btn.addEventListener("click", () => {
...
function showBetaWelcome() {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            qsa(modal, "[data-close]").forEach((b) => b.removeEventListener("click", onClose));
            resolve();
        };
        ...
        qsa(modal, "[data-close]").forEach((b) => b.addEventListener("click", onClose));
        modal.classList.add("open");
    });
}
...
async function initTour(force = false) {
    ...
    const cleanup = (run = false) => {
        ...
        const wc = qsa(welcome, "[data-close]");
...
const bd = welcome.querySelector(".modal-backdrop");
        ...
        const wc2 = qsa(welcome, "[data-close]");
...
async function runTour(startStep = 0) {
    ...
    function applySpotlight(rect, settingsMode = false) {
        const spots = qsa(document, ".tour-spotlight");
