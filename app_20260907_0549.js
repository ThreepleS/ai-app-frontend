...
const APP_VERSION = "20260907_0549";
    ...
    try {
        ...
        const flushLine = async (line) => {
            ...
            if (ev.type === "start") {
                ...
                if (!finalContent) {
                    if (botEl) botEl.remove();
                    addMessage("bot", "<i data-lucide='alert-circle' class='lucide'></i> Модель вернула пустой ответ. Возможно, на стороне провайдера высокая нагрузка или лимиты. Попробуйте повторить запрос или выбрать другую модель.");
                    notify();
                    return;
                }
                if (thinkingEl) {
