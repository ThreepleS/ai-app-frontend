...
// === Глобальная функция qsa (гарантированно работает везде) ===
// Возвращает настоящий Array через ручной цикл — работает в любом WebView.
function qsa(selector, root) {
    root = root || document;
    var list;
    try { list = root.querySelectorAll(selector); }
    catch (e) { return []; }
    if (!list) return [];
    var arr = [];
...
dialogs.forEach((d) => {
            ...
            if (window.lucide) lucide.createIcons();
            documentqsa(".dialog-item-del").forEach((btn) => {
            ...
            documentqsa(".dialog-item-export").forEach((btn) => {
...
async function autoSaveCurrentDialog() {
    ...
    saveTimer = setTimeout(async () => {
        ...
        try {
            ...
            if (saved && saved.name) {
                const items = documentqsa(".dialog-item");
                items.forEach((item) => {
...
function updateEmptyState() {
    const isEmpty = boxqsa(".msg").length === 0;
    if (isEmpty) {
...
if (fsEditor) {
    fsEditorqsa("[data-close]").forEach((el) => {
        ...
        if (tourActive && localStorage.getItem(TOUR_KEY) === "true") {
            ...
            if (congrats) {
                ...
                const close = () => {
                    ...
                    congratsqsa("[data-close]").forEach((b) => b.removeEventListener("click", onOk));
                    closeSettings();
                    runTour(8);
                };
                const onOk = () => close();
                ...
                congratsqsa("[data-close]").forEach((b) => b.addEventListener("click", onOk));
            }
        }
    } catch (err) {
        status.style.color = "#e06b6b";
...
documentqsa(".seg-picker").forEach((picker) => {
...
function clearSearchHighlight() {
    ...
    boxqsa(".search-highlight-term").forEach((el) => {
...
function openSettings(tab = null) {
    ...
    loadKeyInfo();
    if (tab) {
        documentqsa(".stab").forEach(t => t.classList.remove("active"));
        documentqsa(".tab-content").forEach(c => c.classList.remove("active"));
...
// --- Settings tabs ----------------------------------------------------
documentqsa(".settings-tabs .stab").forEach((tab) => {
...
function showConfirm(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            modalqsa("[data-close]").forEach((btn) => btn.removeEventListener("click", onCancel));
            modal.querySelector(".modal-backdrop").removeEventListener("click", onCancel);
        };
        $("#confirmOk").addEventListener("click", onOk);
        modalqsa("[data-close]").forEach((btn) => btn.addEventListener("click", onCancel));
...
function showAlert(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            modal.classList.remove("open");
            modalqsa("[data-close]").forEach((btn) => btn.removeEventListener("click", onClose));
        ...
        modalqsa("[data-close]").forEach((btn) => btn.addEventListener("click", onClose));
...
documentqsa("[data-close]").forEach((btn) => {
    btn.addEventListener("click", () => {
...
function showBetaWelcome() {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            modalqsa("[data-close]").forEach((b) => b.removeEventListener("click", onClose));
            resolve();
        };
        ...
        modalqsa("[data-close]").forEach((b) => b.addEventListener("click", onClose));
        modal.classList.add("open");
    });
}
...
async function initTour(force = false) {
    ...
    const cleanup = (run = false) => {
        ...
        const wc = welcomeqsa("[data-close]");
...
const bd = welcome.querySelector(".modal-backdrop");
        ...
        const wc2 = welcomeqsa("[data-close]");
...
async function runTour(startStep = 0) {
    ...
    function applySpotlight(rect, settingsMode = false) {
        const spots = documentqsa(".tour-spotlight");
