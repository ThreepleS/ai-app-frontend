...
const APP_VERSION = "20260909_0943";
...
// aiHubQueryAll — уникальное имя, collision-proof. Всегда возвращает настоящий Array.
function aiHubQueryAll(root, selector) {
    if (!root || typeof selector !== "string") return [];
    try {
        const nodes = root.querySelectorAll(selector);
        const out = [];
        for (let i = 0; i < nodes.length; i++) out.push(nodes[i]);
        if (typeof out.forEach !== "function") {
            out.forEach = function(callback, thisArg) {
...
// Polyfill forEach на списковых прототипах (на случай прямых вызовов нативного querySelectorAll)
(function () {
    function fp(C) {
...
// Ensure Array.prototype.forEach always exists (some old WebViews may lack it)
...
window.addEventListener("unhandledrejection", (e) => {
    console.error("[unhandled]", e && e.reason ? (e.reason.message || e.reason) : e);
});
...
let activeRenderedStartIndex = 0;
let isLoadingOlderMessages = false;

function createMessageNode(role, content, image, stats, msgIdx, compressed, isDivider) {
    if (isDivider || role === "system") {
        const div = document.createElement("div");
        div.className = "context-compression-divider";
          ...
          <div class="ccd-inner">
            <div class="ccd-header">
              <div class="ccd-badge">
                ...
                <span>Контекст сжат</span>
              </div>
              <div class="ccd-notice">Сообщения выше не учитываются в контексте нейросети и доступны только для чтения</div>
            </div>
            <div class="ccd-summary-card">
              <div class="ccd-summary-header">
                <div class="ccd-summary-title">
                  <i data-lucide="file-text" class="icon"></i>
                  <span>Сохранённый контекст диалога</span>
                </div>
                <button type="button" class="ccd-toggle-btn" title="Свернуть / развернуть резюме">
                  <i data-lucide="chevron-down" class="icon"></i>
                </button>
              </div>
              <div class="ccd-summary-content md">${renderMarkdown(content || "")}</div>
            </div>
          </div>
        `;
        const card = div.querySelector(".ccd-summary-card");
        const head = div.querySelector(".ccd-summary-header");
        if (head && card) {
    ...
    if (compressed) {
        el.classList.add("msg-compressed");
        el.dataset.compressed = "true";
    }
    el.innerHTML = html;
    if (msgIdx != null) el.dataset.msgIdx = String(msgIdx);
    if (role === "bot" || role === "assistant") {
        if (content) el.dataset.raw = content;
        if (stats) {
            el.dataset.stats = typeof stats === "object" ? JSON.stringify(stats) : String(stats);
...
function loadOlderMessagesChunk() {
    if (isLoadingOlderMessages || activeRenderedStartIndex <= 0) return;
    isLoadingOlderMessages = true;
    const msgs = (currentDialogData && Array.isArray(currentDialogData.messages)) ? currentDialogData.messages : [];
    const newStart = Math.max(0, activeRenderedStartIndex - CHAT_PAGE_SIZE);
    const fragment = document.createDocumentFragment();
    for (let i = newStart; i < activeRenderedStartIndex; i++) {
        const m = msgs[i];
        const node = createMessageNode(m.role, m.content || "", m.image || null, m.stats || null, i, m.compressed, m.is_compression_divider);
    ...
    if (sentinel && sentinel.nextSibling) {
        box.insertBefore(fragment, sentinel.nextSibling);
    } else if (sentinel) {
        box.appendChild(fragment);
    } else {
        box.insertBefore(fragment, box.firstChild);
    }
    activeRenderedStartIndex = newStart;
    if (sentinel) {
        if (activeRenderedStartIndex <= 0) {
            sentinel.remove();
        } else {
            const btn = sentinel.querySelector(".chat-load-more-btn");
            if (btn) btn.innerHTML = `<i data-lucide="chevrons-up"></i> Предыдущие сообщения (${activeRenderedStartIndex})`;
        }
    }
    if (window.lucide) lucide.createIcons({ root: box });
    const heightDiff = box.scrollHeight - oldScrollHeight;
    box.scrollTop = oldScrollTop + heightDiff;
    isLoadingOlderMessages = false;
}

function jumpToDialogMessage(targetIdx, term) {
    if (targetIdx < activeRenderedStartIndex) {
        const msgs = (currentDialogData && Array.isArray(currentDialogData.messages)) ? currentDialogData.messages : [];
        const newStart = Math.max(0, targetIdx - 5);
        const fragment = document.createDocumentFragment();
        for (let i = newStart; i < activeRenderedStartIndex; i++) {
            const m = msgs[i];
            const node = createMessageNode(m.role, m.content || "", m.image || null, m.stats || null, i, m.compressed, m.is_compression_divider);
        ...
        activeRenderedStartIndex = newStart;
        if (sentinel) {
            if (activeRenderedStartIndex <= 0) sentinel.remove();
            else {
                const btn = sentinel.querySelector(".chat-load-more-btn");
                if (btn) btn.innerHTML = `<i data-lucide="chevrons-up"></i> Предыдущие сообщения (${activeRenderedStartIndex})`;
            }
    ...
    const targetEl = box.querySelector(`.msg[data-msg-idx="${targetIdx}"]`);
    if (targetEl) {
        targetEl.classList.add("search-highlight-frame");
        highlightTermsInMsg(targetEl, term);
...
function renderDialog(dialog) {
    ...
    setTimeout(() => {
        box.innerHTML = "";
        const msgs = Array.isArray(dialog.messages) ? dialog.messages : [];
        activeRenderedStartIndex = Math.max(0, msgs.length - CHAT_PAGE_SIZE);
        const fragment = document.createDocumentFragment();
        if (activeRenderedStartIndex > 0) {
            const sentinel = document.createElement("div");
            sentinel.id = "chatTopSentinel";
            sentinel.className = "chat-top-sentinel";
            sentinel.innerHTML = `<button type="button" class="btn btn-ghost chat-load-more-btn" style="width:100%;font-size:12px;padding:8px 12px;color:var(--muted);display:flex;align-items:center;justify-content:center;gap:6px;background:none;border:none;cursor:pointer;"><i data-lucide="chevrons-up"></i> Предыдущие сообщения (${activeRenderedStartIndex})</button>`;
            sentinel.querySelector(".chat-load-more-btn").addEventListener("click", () => {
                loadOlderMessagesChunk();
            });
            fragment.appendChild(sentinel);
        }
        for (let i = activeRenderedStartIndex; i < msgs.length; i++) {
            const m = msgs[i];
            const node = createMessageNode(m.role, m.content || "", m.image || null, m.stats || null, i, m.compressed, m.is_compression_divider);
            fragment.appendChild(node);
        }
...
dialogs.forEach((d) => {
                ...
                if (!hasStats && messages.length > 0) {
                    messages.forEach((m) => {
                        if (m.content) totalTokens += estimateTokens(m.content);
                    });
                }
                ...
                const startRename = () => {
                    ...
                    const finish = () => {
                        saveDialogToDb(
                            Object.assign({}, d, { name: inp.value || d.name })
...
function collectMessagesFromDom() {
    const msgs = [];
    aiHubQueryAll(document, ".msg, .context-compression-divider").forEach((el) => {
        if (el.classList.contains("context-compression-divider")) {
            const sumContent = el.querySelector(".ccd-summary-content");
            const content = sumContent ? sumContent.innerText || sumContent.textContent : "";
            msgs.push({
                role: "system",
                is_compression_divider: true,
                content: content.trim(),
                created_at: Date.now()
            });
            return;
        }
        if (el.classList.contains("error-msg") || el.querySelector(".chat-error-card")) return;
        ...
        if (el.classList.contains("msg-compressed") || el.dataset.compressed === "true") {
            msgObj.compressed = true;
        }
...
function updateContextStats() {
    ...
    const msgs = (currentDialogData && Array.isArray(currentDialogData.messages) && currentDialogData.messages.length > 0)
        ? currentDialogData.messages
        : null;

    if (msgs) {
        msgs.forEach((m, idx) => {
            const isDivider = m.is_compression_divider;
            ...
            // 1. Cumulative Dialogue Totals (ALL messages in this dialog, including compressed)
            if (isBot) {
                if (stats) {
                ...
                const nextHasPrompt = nextMsg && (nextMsg.role === "bot" || nextMsg.role === "assistant") && nextMsg.stats && nextMsg.stats.prompt_tokens;
                if (!nextHasPrompt) {
                    sent += t;
                }
            }

            // 2. Active Context (Used) — ONLY uncompressed messages & active compression divider
            if (!m.compressed) {
                if (isDivider) {
                    used += t;
        ...
        const domItems = aiHubQueryAll(box, ".msg, .context-compression-divider");
        domItems.forEach((el, idx) => {
            const isDivider = el.classList.contains("context-compression-divider");
            const isCompressed = el.classList.contains("msg-compressed") || el.dataset.compressed === "true";
            const isUser = el.classList.contains("user");
            const isBot = el.classList.contains("bot");

            if (isDivider) {
                const sumContent = el.querySelector(".ccd-summary-content");
                const text = sumContent ? (sumContent.innerText || sumContent.textContent || "") : "";
            ...
            const md = el.querySelector(".md");
            const text = (md ? (md.innerText || md.textContent || "") : (el.innerText || el.textContent || "")).trim();
            const t = estimateTokens(text);
            let stats = null;
            if (isBot && el.dataset.stats) {
                try { stats = JSON.parse(el.dataset.stats); } catch (e) {}
            }

            // 1. Cumulative Dialogue Totals
            if (isBot) {
                if (stats) {
                    if (stats.prompt_tokens) sent += stats.prompt_tokens;
                    if (stats.completion_tokens) recv += stats.completion_tokens;
                ...
                const nextEl = domItems[idx + 1];
                const nextHasPrompt = nextEl && nextEl.classList.contains("bot") && nextEl.dataset.stats;
                if (!nextHasPrompt) {
                    sent += t;
                }
            }

            // 2. Active Context (Used)
            if (!isCompressed) {
                used += t;
            }
        });
...
let saveTimer = null;
async function autoSaveCurrentDialog(immediate = false) {
    if (!activeDialogId) return;
    if (saveTimer) clearTimeout(saveTimer);
    const doSave = async () => {
        let msgs = [];
        if (currentDialogData && Array.isArray(currentDialogData.messages) && currentDialogData.messages.length > 0) {
            msgs = currentDialogData.messages;
        } else {
            msgs = collectMessagesFromDom();
            if (currentDialogData) currentDialogData.messages = msgs;
        }
...
function rerenderAllStats(targetMode) {
    const mode = targetMode || (($("#s_stats") && $("#s_stats").value) || "full");
    aiHubQueryAll(box, ".msg.bot").forEach((botMsg) => {
        let statsObj = null;
        if (botMsg.dataset.stats) {
            try { statsObj = JSON.parse(botMsg.dataset.stats); } catch (e) {}
        }
        let el = botMsg.querySelector(".stats");
        if (!statsObj || mode === "disabled") {
...
function sanitizeMdHtml(htmlStr) {
    if (!window.DOMPurify) {
        console.warn("[sanitizeMdHtml] DOMPurify is not available, escaping HTML as secure fallback");
        return escapeHtml(htmlStr);
    }
    return DOMPurify.sanitize(htmlStr, {
        ALLOWED_TAGS: Array.from(MD_ALLOWED_TAGS).map((t) => t.toLowerCase()),
        ALLOWED_ATTR: ["href", "src", "alt", "class", "open", "data-lucide"],
...
function renderMarkdown(src) {
    try {
        ...
        // 0) выносим размышления в заглушки, чтобы marked не перемешал их с основным текстом
        const thinks = [];
        let pre = text.replace(/<(?:think|thought|reasoning)>([\s\S]*?)(?:<\/(?:think|thought|reasoning)>|$)/gi, (_, thinkText) => {
        ...
        // 5) возвращаем размышления (в виде аккордеона)
        html = html.replace(
            /(?:<p>)?\u0002T(\d+)T\u0002(?:<\/p>)?/g,
            (_, i) => {
                const rawThink = thinks[Number(i)] || "";
                let thinkBody = "";
                try {
                    thinkBody = typeof marked !== "undefined" && marked.parse
                        ? sanitizeMdHtml(marked.parse(rawThink, { gfm: true, breaks: true }))
                        : rawThink.split("\n").map(esc).join("<br>");
                } catch (_) {
                    thinkBody = rawThink.split("\n").map(esc).join("<br>");
                }
                return `<details class="thinking-block"><summary class="thinking-summary"><span class="thinking-title"><i data-lucide="brain" class="thinking-icon"></i><span>Размышления</span></span><i data-lucide="chevron-down" class="thinking-arrow"></i></summary><div class="thinking-content">${thinkBody}</div></details>`;
            }
        );
        return html;
    } catch (_) {
...
function applyTheme(t) {
    ...
    if (grid) {
        aiHubQueryAll(document, ".theme-card").forEach((c) => {
...
function updateEmptyState() {
    const isEmpty = aiHubQueryAll(document, ".msg").length === 0;
    if (isEmpty) {
...
function updateInputState() {
    const bar = $("#bar");
    const input = $("#input");
    const sendBtn = bar ? bar.querySelector(".send-btn") : null;
    const imgBtn = $("#imgbtn");
    if (!bar || !input) return;
    if (needsKey) {
...
function parseErrorDetails(rawInput, status) {
    const raw = typeof rawInput === "string" ? rawInput : JSON.stringify(rawInput, null, 2);
    let title = "Ошибка при запросе";
    let friendly = "";

    if (status === 429 || /429|rate[-_]?limit|upstream_provider_shared_pool|quota/i.test(raw)) {
        title = "Модель перегружена (Лимит запросов)";
    ...
    } else if (status === 402 || /402|insufficient[-_]?funds|credits|balance/i.test(raw)) {
        title = "Недостаточно средств / квоты";
        friendly = "Исчерпан лимит баланса или бесплатных токенов для этой модели. Выберите бесплатную модель или укажите свой ключ в настройках.";
    } else if (status === 401 || status === 403 || /401|403|unauthorized|forbidden|api[-_]?key|invalid key/i.test(raw)) {
        title = "Ошибка доступа к модели";
        friendly = "Недействительный или отсутствующий API-ключ. Проверьте ключ в настройках или переключитесь на общедоступную модель.";
    } else if (status === 502 || status === 503 || status === 504 || /502|503|504|overloaded|upstream|unavailable|gateway/i.test(raw)) {
        title = "Провайдер временно недоступен";
        friendly = "Сервер провайдера сейчас не отвечает или испытывает сбои. Попробуйте отправить запрос снова чуть позже.";
    } else if (status === 404 || /404|not[-_]?found/i.test(raw)) {
        title = "Модель недоступна";
        friendly = "Выбранная модель устарела или временно отключена у провайдера. Рекомендуем выбрать другую модель из каталога.";
    } else {
        title = "Ошибка модели";
        friendly = "Не удалось получить ответ от выбранной модели. Попробуйте повторить запрос или переключиться на другую модель.";
    }

    return { title, friendly, raw };
}

function renderErrorHtml(title, friendlyMsg, rawDetails) {
    const raw = String(rawDetails || "").trim();
    const rawBlock = raw ? `
        <details class="error-details">
            <summary class="error-summary">
                <span class="error-summary-title">
                    <i data-lucide="terminal" class="error-summary-icon"></i>
                    <span>Технические детали ошибки</span>
                </span>
                <i data-lucide="chevron-down" class="error-summary-arrow"></i>
            </summary>
            <pre class="error-raw"><code>${esc(raw)}</code></pre>
        </details>
    ` : "";
    ...
    return `
        <div class="chat-error-card">
            <div class="chat-error-header">
                <i data-lucide="alert-circle" class="chat-error-icon"></i>
                <div class="chat-error-body">
                    <div class="chat-error-title">${esc(title || "Внимание")}</div>
                    <div class="chat-error-msg">${esc(friendlyMsg)}</div>
                </div>
            </div>
            ${rawBlock}
        </div>
...
function addErrorMessage(title, friendlyMsg, rawDetails) {
    const html = renderErrorHtml(title, friendlyMsg, rawDetails);
    const el = addMessage("bot error-msg", html);
    if (window.lucide) lucide.createIcons({ root: el });
    return el;
}

function addCodeCopy(root) {
    aiHubQueryAll(document, "pre").forEach((pre) => {
...
// Рендер одного сообщения из истории.
function addHistoryMessage(role, content, image, stats, scroll = false) {
    const node = createMessageNode(role, content, image, stats);
...
function fillSettings(s) {
    ...
    let limitMode = s.context_limit_mode;
    if (!limitMode) {
        try {
            limitMode = localStorage.getItem("context_limit_mode");
        } catch (_) {}
    }
    if (!limitMode) {
        limitMode = (s.context_limit && Number(s.context_limit) > 100) ? "tokens" : "messages";
    }
    updateContextLimitSliderMode(limitMode, s.context_limit);
...
async function auth(devId) {
    ...
    try {
        ...
        if (isAdmin) {
            ...
            try {
                localStorage.removeItem("erudite_enabled");
...
const imgBtnEl = $("#imgbtn");
if (imgBtnEl) {
    imgBtnEl.addEventListener("click", () => openAttachSheet());
}

const attachSheetCloseBtn = $("#attachSheetClose");
if (attachSheetCloseBtn) {
    attachSheetCloseBtn.addEventListener("click", closeAttachSheet);
}

const attachSheetBackdropEl = $("#attachSheetBackdrop");
if (attachSheetBackdropEl) {
    attachSheetBackdropEl.addEventListener("click", closeAttachSheet);
}

const attachOptGalleryBtn = $("#attachOptGallery");
if (attachOptGalleryBtn) {
    attachOptGalleryBtn.addEventListener("click", () => {
        closeAttachSheet();
        const inp = $("#imgfile");
        if (inp) inp.click();
    });
...
const attachOptCameraBtn = $("#attachOptCamera");
if (attachOptCameraBtn) {
    attachOptCameraBtn.addEventListener("click", () => {
        closeAttachSheet();
        const inp = $("#imgfile_camera");
        if (inp) inp.click();
    });
}
...
if (imgFileEl) {
    imgFileEl.addEventListener("change", handleImageFileInput);
}

const imgFileCameraEl = $("#imgfile_camera");
if (imgFileCameraEl) {
    imgFileCameraEl.addEventListener("change", handleImageFileInput);
}
...
if (fsEditor) {
    aiHubQueryAll(document, "[data-close]").forEach((el) => {
...
async function compressContextNow() {
    const confirmed = await showConfirm(
        "Сжать контекст?",
        "Старые сообщения будут сохранены только для чтения и исключены из активного контекста нейросети. Продолжить?"
    );
    if (!confirmed) return;

    // Flush any pending unsaved messages to DB first
    if (saveTimer) {
    ...
    const msgs = (currentDialogData && Array.isArray(currentDialogData.messages) && currentDialogData.messages.length > 0)
        ? currentDialogData.messages
        : collectMessagesFromDom();
    if (activeDialogId && msgs.length > 0) {
        const currentDialog = currentDialogData || {};
        await saveDialogToDb({
            id: activeDialogId,
            messages: msgs,
            model: currentModelId || "",
    ...
    try {
        const res = await ef("chat", { compress: true, dialog_id: activeDialogId }, 60000);
        const data = await res.json();
        if (data.ok) {
            toast(data.message || "Контекст успешно сжат", "ok");
            // Reload current dialog to show new history with compression divider
            await ensureCurrentDialog();
            setTimeout(() => {
...
function showCtx(x, y, msgEl) {
    ...
    ctxMsgEl = msgEl;
    const isCompressed = msgEl.classList.contains("msg-compressed") || msgEl.dataset.compressed === "true";
    const replyBtn = ctxMenu.querySelector('[data-act="reply"]');
    const regenBtn = ctxMenu.querySelector('[data-act="regen"]');
    if (replyBtn) replyBtn.style.display = isCompressed ? "none" : "";
    if (regenBtn) regenBtn.style.display = isCompressed ? "none" : "";
...
$("#bar").addEventListener("submit", async (e) => {
    ...
    const userMsgEl = addMessage("user", userHtml + imgHtml);
    ...
    if (currentDialogData && Array.isArray(currentDialogData.messages)) {
        userMsgEl.dataset.msgIdx = String(currentDialogData.messages.length);
        currentDialogData.messages.push({ role: "user", content: text, image: imageToSend });
    }
    ...
    try {
        ...
        const fullHist = (currentDialogData && Array.isArray(currentDialogData.messages) && currentDialogData.messages.length > 0)
            ? currentDialogData.messages
            : collectMessagesFromDom();
        let currentHist = fullHist.length && fullHist[fullHist.length - 1].role === "user"
            ? fullHist.slice(0, -1)
            : fullHist;

        // Filter out compressed messages and handle latest compression divider
        let lastDividerIdx = -1;
        for (let i = currentHist.length - 1; i >= 0; i--) {
            if (currentHist[i].is_compression_divider) {
                lastDividerIdx = i;
                break;
            }
        }
        if (lastDividerIdx !== -1) {
            const divider = currentHist[lastDividerIdx];
            const afterDivider = currentHist.slice(lastDividerIdx + 1).filter((m) => !m.compressed);
            currentHist = [
                { role: "assistant", content: "Резюме предыдущего контекста диалога:\n" + (divider.content || "").trim() },
                ...afterDivider
            ];
        } else {
            currentHist = currentHist.filter((m) => !m.compressed);
        }
        ...
        if (!res.ok) {
            ...
            addErrorMessage(data.title || title, data.message || friendly, data.raw || raw || errStr);
            await autoSaveCurrentDialog();
            return;
        }
        ...
        const flushLine = async (line) => {
            ...
            if (ev.type === "start") {
                ...
                if (!thinkingEl) {
                    ...
                    thinkingEl.innerHTML = `<summary class="thinking-summary"><span class="thinking-title"><i data-lucide="brain" class="thinking-icon"></i><span>Размышления</span><span class="thinking-badge"><span class="thinking-pulse"></span> рассуждает...</span></span><i data-lucide="chevron-down" class="thinking-arrow"></i></summary><div class="thinking-content"></div>`;
                    thinkingContentEl = thinkingEl.querySelector(".thinking-content");
                    if (botEl) botEl.insertBefore(thinkingEl, botText);
                    if (window.lucide) lucide.createIcons({ root: thinkingEl });
                }
                ...
                full += ev.text || "";
                const thinkOpenIdx = full.indexOf("<think>");
                const thinkCloseIdx = full.indexOf("</think>");

                if (thinkOpenIdx !== -1 && thinkCloseIdx === -1) {
                    inbandThinking = true;
                    const tText = full.slice(thinkOpenIdx + 7);
                    if (!thinkingEl) {
                        thinkingEl = document.createElement("details");
                        thinkingEl.className = "thinking-block";
                        thinkingEl.open = true;
                    ...
                    if (thinkingContentEl) thinkingContentEl.textContent = tText;
                    if (botText) botText.textContent = "";
                } else if (thinkOpenIdx !== -1 && thinkCloseIdx !== -1) {
                    if (inbandThinking) {
                        inbandThinking = false;
                        if (thinkingEl) {
                            thinkingEl.open = false;
                    ...
                    const afterThink = full.slice(thinkCloseIdx + 8).trimStart();
                    if (botText) botText.textContent = afterThink;
                } else {
                ...
                const { title, friendly, raw } = parseErrorDetails(ev.raw || ev.message, ev.status);
                ...
                if (!finalContent) {
                    ...
                    addErrorMessage(
                        ...
                        "Модель вернула пустой ответ. Возможно, на стороне провайдера высокая нагрузка или лимиты. Попробуйте повторить запрос или выбрать другую модель.",
                        "Empty response content from provider API"
                    );
                    notify();
                    return;
                }
                ...
                if (botText) {
                    ...
                    if (window.lucide) lucide.createIcons({ root: botText });
                }
                if (botEl) botEl.dataset.raw = ev.markdown || full;
                const usage = ev.usage || {};
                const pt = usage.prompt_tokens ?? usage.promptTokenCount ?? null;
                const ct = usage.completion_tokens ?? usage.candidatesTokenCount ?? null;
                const tt = usage.total_tokens ?? usage.totalTokenCount ?? (pt != null || ct != null ? ((pt || 0) + (ct || 0)) : null);
                const thinking = usage.thinking_tokens ?? usage.thoughtsTokenCount ?? null;
                const cached = usage.cached_tokens ?? usage.cachedContentTokenCount ?? null;
                let statsObj = null;
                ...
                if (currentDialogData && Array.isArray(currentDialogData.messages)) {
                    if (botEl) botEl.dataset.msgIdx = String(currentDialogData.messages.length);
                    currentDialogData.messages.push({ role: "bot", content: ev.markdown || full, stats: statsObj });
                }
        ...
        const { title, friendly, raw } = parseErrorDetails(String(err?.message || err));
        addErrorMessage(title, friendly, String(err?.stack || err));
        notify();
    } finally {
...
function addToRecommended(id) {
    ...
    RECOMMENDED_MODELS = [id].concat(RECOMMENDED_MODELS);
    mbRenderList();
    ...
    try {
        ...
        for (const k in mbState.working) {
            if (mbState.working[k] instanceof Set)
                working[k] = Array.from(mbState.working[k]);
        }
        localStorage.setItem(
            mbPingStoreKey(),
            JSON.stringify({
...
function mbModelsForGroup(gkey) {
    ...
    if (gkey === "recommended") {
        ...
        if (cached.length > 0) return [autofreeItem].concat(cached);
        ...
        return [autofreeItem].concat(items);
    }
...
function mbClose(force = false) {
    if (tourActive && !force) return;
...
$("#s_save").addEventListener("click", async () => {
    ...
    let contextLimit = 10;
    if (limitMode === "tokens") {
        contextLimit = parseInt($("#s_limit").value, 10);
        if (isNaN(contextLimit) || contextLimit < 1000) contextLimit = 10000;
        contextLimit = Math.max(1000, Math.min(1000000, contextLimit));
    } else if (limitMode === "messages") {
        contextLimit = parseInt($("#s_limit").value, 10);
    ...
    try {
        ...
        localStorage.setItem("context_limit_mode", limitMode);
    ...
    try {
        ...
        updateEmptyState();
        updateInputState();
        if (data.settings && data.settings.selected_model) {
            log("модель: " + data.settings.selected_model);
        }
        if (tourActive && localStorage.getItem(TOUR_KEY) === "true") {
            ...
            if (congrats) {
                ...
                const close = () => {
                    ...
                    aiHubQueryAll(document, "[data-close]").forEach((b) => b.removeEventListener("click", onOk));
                    closeSettings();
                    runTour(6);
                };
                const onOk = () => close();
                ...
                aiHubQueryAll(document, "[data-close]").forEach((b) => b.addEventListener("click", onOk));
            }
        }
    } catch (err) {
        status.style.color = "#e06b6b";
...
let contextLimitSaveTimer = null;
function saveContextLimitNow() {
    clearTimeout(contextLimitSaveTimer);
    contextLimitSaveTimer = null;
    const limitMode = localStorage.getItem("context_limit_mode") || "messages";
    let contextLimit = 10;
    if (limitMode === "tokens") {
        contextLimit = parseInt($("#s_limit") ? $("#s_limit").value : "10000", 10);
        if (isNaN(contextLimit) || contextLimit < 1000) contextLimit = 10000;
        contextLimit = Math.max(1000, Math.min(1000000, contextLimit));
    } else if (limitMode === "messages") {
        contextLimit = parseInt($("#s_limit") ? $("#s_limit").value : "10", 10);
        if (isNaN(contextLimit) || contextLimit < 1) contextLimit = 10;
        contextLimit = Math.max(1, Math.min(100, contextLimit));
    }
    try {
        ef("settings", Object.assign(authBody(), {
            context_limit: contextLimit,
            context_limit_mode: limitMode
        }), 10000);
    } catch (e) {}
...
function debounceSaveContextLimit() {
    clearTimeout(contextLimitSaveTimer);
    contextLimitSaveTimer = setTimeout(saveContextLimitNow, 800);
}
...
aiHubQueryAll(document, ".seg-picker").forEach((picker) => {
    picker.addEventListener("click", (e) => {
        ...
        if (name === "context_limit_mode") {
            updateContextLimitSliderMode(newVal);
            debounceSaveContextLimit();
        } else {
            syncSegPickers();
            if (name === "stats_display") {
                rerenderAllStats(newVal);
                try {
                    ef("settings", Object.assign(authBody(), { stats_display: newVal }), 10000);
                } catch (e) {}
            }
        }
    });
...
const vibCheck = $("#s_vibrate");
if (vibCheck) {
    vibCheck.addEventListener("change", async () => {
        const checked = vibCheck.checked;
        try {
            const ls = JSON.parse(localStorage.getItem("local_settings") || "{}");
            ls.notify_vibrate = checked;
            localStorage.setItem("local_settings", JSON.stringify(ls));
            await ef("settings", Object.assign(authBody(), {
                notify_vibrate: checked
            }), 10000);
        } catch (e) {}
    });
}
...
if (limitSlider) {
    limitSlider.addEventListener("input", () => {
        const mode = localStorage.getItem("context_limit_mode") || "messages";
        const val = parseInt(limitSlider.value, 10);
        if (mode === "tokens") {
            if ($("#limitVal")) $("#limitVal").textContent = formatNum(val);
            try { localStorage.setItem("context_limit_tokens", String(val)); } catch (_) {}
        } else {
            if ($("#limitVal")) $("#limitVal").textContent = String(val);
            try { localStorage.setItem("context_limit_msgs", String(val)); } catch (_) {}
        }
        if ($("#s_limit")) $("#s_limit").value = String(val);
        debounceSaveContextLimit();
    });
}
...
function saveSystemPromptNow() {
    ...
    systemPromptSaveTimer = null;
    const sp = ($("#s_prompt") ? $("#s_prompt").value : "").trim();
    try {
        ef("settings", Object.assign(authBody(), {
            system_prompt: sp
        }), 15000);
...
function flushPendingSettings() {
    if (systemPromptSaveTimer) {
        saveSystemPromptNow();
    }
    if (contextLimitSaveTimer) {
        saveContextLimitNow();
    }
}
...
if ($("#s_prompt")) {
    $("#s_prompt").addEventListener("input", () => {
        clearTimeout(systemPromptSaveTimer);
        systemPromptSaveTimer = setTimeout(saveSystemPromptNow, 1000);
    });
    $("#s_prompt").addEventListener("blur", () => {
        saveSystemPromptNow();
    });
    $("#s_prompt").addEventListener("change", () => {
        saveSystemPromptNow();
    });
}
...
$("#tplList").addEventListener("click", async (e) => {
    ...
    if (info) {
        ...
        if (t) {
            $("#s_prompt").value = t.text;
            saveSystemPromptNow();
            toast("Промпт применён", "ok");
        }
    }
});
...
function clearSearchHighlight() {
    aiHubQueryAll(box, ".search-highlight-frame").forEach((el) => el.classList.remove("search-highlight-frame"));
...
box.addEventListener("scroll", () => {
    ...
    if (box.scrollTop < 60 && activeRenderedStartIndex > 0 && !isLoadingOlderMessages) {
        loadOlderMessagesChunk();
    }
});
...
function renderSearchResults(q) {
    ...
    items.forEach((it) => {
        ...
        div.addEventListener("click", () => {
            jumpToDialogMessage(it.msgIdx, q);
            closeChatSearch();
        });
...
if (newChatBtn) {
    ...
    try {
        ...
        try {
            ...
            console.warn("[newChat] legacy chat clear warning:", clearErr);
        }
        const created = await createDialogDb();
...
function updateContextLimitSliderMode(mode, initialVal) {
    const picker = document.querySelector('.seg-picker[data-name="context_limit_mode"]');
    if (picker) {
        picker.dataset.value = mode;
    }
    try {
        localStorage.setItem("context_limit_mode", mode);
    } catch (_) {}

    const sliderWrap = picker ? picker.parentElement.querySelector(".range-container") : $(".range-container");
    const limitVal = $("#limitVal");
    const slider = $("#s_limit_slider");
    const hidden = $("#s_limit");

    if (mode === "full") {
        if (sliderWrap) sliderWrap.style.display = "none";
        if (limitVal) limitVal.style.display = "none";
    ...
    if (limitVal) limitVal.style.display = "";
    ...
    if (mode === "messages") {
        if (slider) {
            ...
            slider.max = "100";
            slider.step = "1";
            let val = initialVal !== undefined && !isNaN(parseInt(initialVal, 10)) ? parseInt(initialVal, 10) : parseInt(localStorage.getItem("context_limit_msgs") || slider.value, 10);
            if (isNaN(val) || val < 1 || val > 100) val = 10;
        ...
        if (slider) {
            ...
            slider.max = "128000";
            slider.step = "1000";
            let val = initialVal !== undefined && !isNaN(parseInt(initialVal, 10)) ? parseInt(initialVal, 10) : parseInt(localStorage.getItem("context_limit_tokens") || slider.value, 10);
...
function exportChat(format) {
    const msgs = [];
    aiHubQueryAll(document, ".msg").forEach((el) => {
...
$("#s_erudite").addEventListener("change", () => {
    if (!isAdmin) {
        $("#s_erudite_wrap").style.display = "none";
        try {
            if (typeof eruda !== "undefined" && eruda && eruda._isInit) eruda.destroy();
        } catch (_) {}
        return;
    }
    ...
    try {
        ...
        console.error("[eruda] switch error", e);
    }
...
function updateSettingsActionsVisibility(activeTab) {
    const bar = $("#settingsActionsBar") || document.querySelector(".settings-bottom-bar .actions-bar");
    if (bar) {
        bar.style.display = activeTab === "keys" ? "flex" : "none";
    }
}

function openSettings(tab = null) {
    const avEl = document.getElementById("appVersion");
    ...
    const currentActiveTab = tab || (document.querySelector(".settings-tabs .stab.active") ? document.querySelector(".settings-tabs .stab.active").dataset.tab : "theme");
    updateSettingsActionsVisibility(currentActiveTab);
    if (window.lucide) lucide.createIcons();
}
...
aiHubQueryAll(document, ".settings-tabs .stab").forEach((tab) => {
    tab.addEventListener("click", () => {
        vibClick();
        aiHubQueryAll(document, ".settings-tabs .stab").forEach((t) => t.classList.remove("active"));
        aiHubQueryAll(document, ".tab-content").forEach((c) => c.classList.remove("active"));
        ...
        if (content) content.classList.add("active");
        updateSettingsActionsVisibility(target);
    });
});
(async () => {
...
function showConfirm(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            aiHubQueryAll(document, "[data-close]").forEach((btn) => btn.removeEventListener("click", onCancel));
            modal.querySelector(".modal-backdrop").removeEventListener("click", onCancel);
        };
        $("#confirmOk").addEventListener("click", onOk);
        aiHubQueryAll(document, "[data-close]").forEach((btn) => btn.addEventListener("click", onCancel));
...
function showAlert(title, message) {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            modal.classList.remove("open");
            aiHubQueryAll(document, "[data-close]").forEach((btn) => btn.removeEventListener("click", onClose));
        ...
        aiHubQueryAll(document, "[data-close]").forEach((btn) => btn.addEventListener("click", onClose));
...
function showBetaWelcome() {
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            aiHubQueryAll(document, "[data-close]").forEach((b) => b.removeEventListener("click", onClose));
            resolve();
        };
        ...
        aiHubQueryAll(document, "[data-close]").forEach((b) => b.addEventListener("click", onClose));
        modal.classList.add("open");
    });
}
...
async function initTour(force = false) {
    ...
    const cleanup = (run = false) => {
        ...
        const wc = aiHubQueryAll(document, "[data-close]");
...
const bd = welcome.querySelector(".modal-backdrop");
        ...
        const wc2 = aiHubQueryAll(document, "[data-close]");
...
async function runTour(startStep = 0) {
    ...
    const cutout = $("#tourMaskCutout");
    const border = $("#tourSpotlightBorder");
    const badgeEl = $("#tourStepBadge");
    const dotsEl = $("#tourStepDots");
    if (!backdrop || !tooltip || !cutout) return;
    ...
    const steps = [
        {
            ...
            body: "В нейросетях любой текст измеряется в токенах (1 токен — это примерно 3-4 буквы или полслова).\n\n• Верхняя строка: показывает, сколько памяти диалога сейчас занято. Иконка кубика — кнопка сжатия: если переписка станет слишком длинной, ИИ сделает краткий пересказ и освободит место.\n• Нижняя строка: стрелочки показывают, сколько токенов ушло на ваш вопрос и получено в ответе.",
            padding: 6
        },
        {
            target: "#models",
            title: "Каталог нейросетей",
            body: "Нажмите сюда, чтобы открыть каталог доступных нейросетей (Google Gemini, OpenRouter, Venice и др.). Переключать модель можно в любой момент прямо во время беседы.",
            padding: 6
        },
        {
            target: "#mb_list .mb-group:first-child",
            fallbackTarget: "#mb_list",
            title: "Как выбрать модель",
            body: "Не разбираетесь в параметрах? Не переживайте! Просто выберите любую модель из блока «Рекомендуемые» — там уже собраны самые умные, быстрые и проверенные нейросети.\n\nНа что обращать внимание:\n• «FREE» — бесплатная модель, баланс не нужен.\n• «Vision» — умеет анализировать фото и картинки.\n• «Контекст» — сколько текста модель удерживает в памяти одновременно.\n• Кнопка «Выбрать» активирует модель, а звёздочка добавляет её в избранное.",
            padding: 6,
            isModelBrowser: true
        },
        {
            ...
            body: "Все ваши переписки сохраняются в отдельных чатах. Создавайте новые диалоги под разные задачи (работа, учёба, идеи), чтобы мысли не путались, а полезные ответы не терялись.",
            padding: 6
        },
        {
            target: "#bar",
            title: "Сообщения и файлы",
            body: "Пишите любые вопросы своими словами, как обычному человеку.\n\n• Скрепка позволяет прикрепить фото или скриншот (для моделей с поддержкой Vision).\n• На компьютере: клавиша Enter отправляет запрос, а сочетание Shift + Enter переносит строку.",
            padding: 8
        },
        {
            target: "#s_prompt",
            ...
            body: "Системный промпт — это главная инструкция для нейросети: её роль, правила поведения и стиль общения.\n\nЗдесь вы можете задать, кем должен быть ИИ. Например: «Отвечай кратко и строго по делу», «Ты опытный юрист» или «Объясняй сложные вещи простыми словами для новичка». Модель будет соблюдать эту инструкцию в каждом своём ответе.",
            padding: 6
        },
        {
            target: "#s_keys",
            ...
            body: "API-ключ — это ваш личный ключ доступа к провайдерам нейросетей (OpenRouter, Google Gemini, Venice).\n\n• Зачем он нужен? С личным ключом запросы идут напрямую к серверам провайдера — без очередей, общих лимитов и с доступом к самым мощным платным и бесплатным моделям.\n• Куда вставлять? В поля ниже напротив нужного провайдера. Ключи надёжно шифруются и не передаются посторонним.\n• Если ключа нет — просто общайтесь с бесплатными моделями из «Рекомендуемых»!",
            padding: 6
        },
        {
            target: null,
            title: "Всё готово к общению!",
            body: "Теперь вы знаете всё самое главное! Выбирайте понравившуюся модель из «Рекомендуемых», пишите первый вопрос и наслаждайтесь общением с искусственным интеллектом.",
            isFinal: true
        }
    ];

    let currentStep = Math.max(0, Math.min(steps.length - 1, startStep));

    function updateDots(index) {
        if (badgeEl) badgeEl.textContent = `${index + 1} / ${steps.length}`;
        if (dotsEl) {
            let html = "";
            for (let i = 0; i < steps.length; i++) {
                html += `<span class="tour-dot ${i === index ? "active" : ""}"></span>`;
            }
        ...
        if (nextBtn) {
            if (index === steps.length - 1) {
                nextBtn.innerHTML = "Начать общение <i data-lucide='sparkles' class='icon'></i>";
            } else {
                nextBtn.innerHTML = "Далее <i data-lucide='chevron-right' class='icon'></i>";
            }
        }
    }
    ...
    function positionTooltip(rect) {
        ...
        const tw = tooltip.offsetWidth || 350;
        ...
        let top;
        if (rect.bottom + th + pad < window.innerHeight - 12) {
            top = Math.round(rect.bottom + pad);
        } else if (rect.top - th - pad > 12) {
            top = Math.round(rect.top - th - pad);
        } else {
            top = Math.max(12, Math.min(window.innerHeight - th - 12, Math.round((window.innerHeight - th) / 2)));
        }
    ...
    function applySpotlight(target, step) {
        if (!target) {
            if (cutout) {
        ...
        const pad = step && step.padding !== undefined ? step.padding : 6;
        const x = Math.round(Math.max(0, rect.left - pad));
        const y = Math.round(Math.max(0, rect.top - pad));
        ...
        try {
            ...
            if (br.includes("%") || parseFloat(br) >= Math.min(rect.width, rect.height) / 2 - 2) {
                rx = Math.round(Math.min(w, h) / 2);
            } else {
                const parsed = parseFloat(br);
                rx = isNaN(parsed) ? 12 : Math.max(8, Math.min(32, Math.round(parsed + pad / 2)));
            }
        } catch (_) {
            rx = 12;
        }
        ...
        if (cutout) {
            ...
            cutout.setAttribute("rx", String(rx));
            cutout.setAttribute("ry", String(rx));
        }
        ...
        if (border) {
            ...
            border.style.borderRadius = rx + "px";
        }
    ...
    function closeTour() {
        ...
        if (closeBtn) closeBtn.removeEventListener("click", onSkip);
