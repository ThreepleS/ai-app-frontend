...
const APP_VERSION = "20260909_1057";

// Дефолтный универсальный системный промпт для всех новых пользователей
const UNIVERSAL_SYSTEM_PROMPT = "Ты — умный, внимательный, эрудированный и универсальный ИИ-ассистент. Отвечай грамотно, вежливо, информативно и строго по существу заданного вопроса. Гибко адаптируй свой стиль под контекст пользователя: если нужен совет — давай ясные и практичные рекомендации, если решение задачи или код — пиши понятное, безопасное и готовое решение, если текст — пиши живо, связно и структурированно. Помогай пользователю эффективно решать любые повседневные, творческие и рабочие задачи без лишней воды.";
...
function fillSettings(s) {
    ...
    let promptVal = (s.system_prompt || "").trim();
    if (!promptVal || promptVal === "Ты - полезный и дружелюбный AI-ассистент." || promptVal === "Ты - полезный и дружелюбный AI-ассистент") {
        promptVal = UNIVERSAL_SYSTEM_PROMPT;
        if ($("#s_prompt")) $("#s_prompt").value = promptVal;
        saveSystemPromptNow();
    } else {
        if ($("#s_prompt")) $("#s_prompt").value = promptVal;
    }
...
async function auth(devId) {
    ...
    try {
        ...
        // Приветственное сообщение показываем всем при каждом запуске приложения
