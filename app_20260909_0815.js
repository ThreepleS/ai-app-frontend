...
const APP_VERSION = "20260909_0815";
        ...
        div.innerHTML = `
          <div class="ccd-inner">
            <div class="ccd-header">
              ...
              <div class="ccd-notice">Сообщения выше не учитываются в контексте нейросети и доступны только для просмотра</div>
            </div>
            <div class="ccd-summary-card">
              <div class="ccd-summary-header">
                <div class="ccd-summary-title">
                  <i data-lucide="file-text" class="icon"></i>
                  <span>Краткое резюме диалога</span>
                </div>
                <button type="button" class="ccd-toggle-btn" title="Показать/скрыть резюме">
...
function updateContextStats() {
    ...
    if (msgs) {
        msgs.forEach((m) => {
            if (m.compressed) return;
            if (m.is_compression_divider) {
        ...
        aiHubQueryAll(box, ".msg").forEach((msgEl) => {
            if (msgEl.classList.contains("msg-compressed")) return;
...
function renderRecentAttachments() {
    const wrap = $("#attachRecentWrap");
    const list = $("#attachRecentList");
    if (!wrap || !list) return;
    ...
    if (currentDialogData && Array.isArray(currentDialogData.messages)) {
        for (let i = currentDialogData.messages.length - 1; i >= 0; i--) {
            const img = currentDialogData.messages[i].image;
            if (img && !seen.has(img)) {
                seen.add(img);
                recentUrls.push(img);
            }
            if (recentUrls.length >= 8) break;
        }
    }

    if (recentUrls.length < 8 && Array.isArray(window.__dialogsCache)) {
        for (const d of window.__dialogsCache) {
            if (Array.isArray(d.messages)) {
                for (let i = d.messages.length - 1; i >= 0; i--) {
                    const img = d.messages[i].image;
                    if (img && !seen.has(img)) {
                        seen.add(img);
                        recentUrls.push(img);
                    }
    ...
    recentUrls.forEach((url) => {
        const item = document.createElement("div");
        item.className = "attach-recent-item";
        item.title = "Прикрепить фото";
        const img = document.createElement("img");
        img.src = url;
        img.alt = "";
...
async function compressContextNow() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.ok) {
            toast(data.summary || "Контекст успешно сжат", "ok");
